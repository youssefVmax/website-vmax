...
export default function UserManagement({ userRole, user }: UserManagementProps) {
  ...
  const checkUsernameExists = async (username: string): Promise<boolean> => {
    try {
      ...
      if (!q) return false;
      const response = await fetch(`/api/users?username=${encodeURIComponent(q)}`, {
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
          'Cache-Control': 'no-cache'
      ...
      // Prefer users array if present
      if (Array.isArray(data.users)) {
        return data.users.some((u: any) => (u?.username || '').toLowerCase() === q.toLowerCase())
      }
      // Fallback to single user field
      if (data.user && data.user.username) {
        return String(data.user.username).toLowerCase() === q.toLowerCase()
      }
      return false
    } catch (error) {
  ...
  const handleAddUser = async () => {
    try {
      ...
      // Normalize username and check if it already exists
      const normalizedUsername = formData.username.trim()
      if (normalizedUsername !== formData.username) {
        setFormData(prev => ({ ...prev, username: normalizedUsername }))
      }
      const existingUsers = await checkUsernameExists(normalizedUsername)
      if (existingUsers) {
        setError('Username already exists')
        return
      }

      console.log('Creating user in MySQL with data:', formData)
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage salesman and team leader.
          </p>
        </div>
        <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
          ...
          <DialogContent className="sm:max-w-[425px]">
            <DialogHeader>
              ...
              <DialogDescription>
                Create a new salesman or team leader account in MySQL.
              </DialogDescription>
            </DialogHeader>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Users Company </CardTitle>
          <CardDescription>
                      ...
                      <div className="flex items-center gap-2">
                        <span className="font-mono text-sm">
                          {showPassword[user.id]
                            ? (user.password && user.password.length > 0 ? user.password : '(not set)')
                            : '••••••••'}
                        </span>
                        <Button
                          variant="ghost"
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Update user information and credentials in MySQL.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
